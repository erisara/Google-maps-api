...
Public Class Form1
    ...
    Private PopSize As Double = 100
    Private MaxGenerations As Double = 500
    Private ProbCrossOver As Double = 0.8
    Private ProbInversion As Double = 0.5
    Private ProbMutation As Double = 0.01
    ...
    Private Function InitializeMap(Optional ByVal fpath As String = "cities.csv") As Boolean
        Dim xCity As City
        Dim noLine As Double = 1
    ...
    '
    '************************************************************************************
    Public Sub RunGA(Optional ByVal csv As String = "cities.csv", Optional ByVal fout As String = "output.txt", Optional ByVal drawform As Boolean = True)
        Dim j As Double
        ...
        Using sw As New StreamWriter(fout)
            ...
            For j = 1 To MaxGenerations
                ...
                If drawform Then
                    ...
                    sw.WriteLine(j.ToString() & ")  " & ShortestPath.PathToString(",", True, "  :  "))
                Else
                    sw.WriteLine(j.ToString() & ")  " & ShortestPath.PathToString(",", True, "  :  "))
                End If
            Next
    ...
    '*
    '************************************************************************************
    Public Sub shiftRight(ByRef path As List(Of Double), ByVal pos As Double)
        Dim i As Double
    ...
    Public Sub OrderCrossOver(ByVal parent1 As Double, ByVal parent2 As Double)
        ...
        Do
            pos1 = CDbl(Int((Map.Count * Rnd()) + 1)) - 1
            pos2 = CDbl(Int((Map.Count * Rnd()) + 1)) - 1
    ...
    Public Sub CrossOver(Optional ByVal doPmx As Boolean = True)
        Dim i, plithos As Double
        Dim parent1, parent2 As Double

        plithos = CInt(ProbCrossOver * PopSize)

        For i = 1 To plithos
            Do
                parent1 = CDbl(Int((PopSize * Rnd()) + 1)) - 1
                parent2 = CDbl(Int((PopSize * Rnd()) + 1)) - 1
    ...
    '************************************************************************************
    Public Sub ChromosomeInversion(ByVal Index As Double)
        Dim pos1, pos2, tmp As Double
    ...
    Public Sub Inversion()
        Dim i, plithos As Double
        Dim parent1 As Double

        plithos = CDbl(ProbInversion * PopSize)
    ...
    '************************************************************************************
    Public Sub ChromosomeMutation(ByVal Index As Double)
        Dim pos1, pos2, tmp As Double
    ...
    Public Sub Mutation()
        ...
        plithos = CDbl(ProbMutation * PopSize)
    ...
    Private Sub CreateNewPopulation(Optional ByVal RankOrder As Boolean = True)
        ...
        For Each thesi As Double In newPopIndexes
            tmp.Add(Population.Item(thesi))
        Next
